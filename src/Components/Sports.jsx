...
const SportsPage = () => {
  ...
  useEffect(() => {
    const fetchSportsNews = async () => {
      try {
        setLoading(true);
        let cachedData = localStorage.getItem(`sportsNews-${searchQuery}`);
        ...
        if (
          ...
          let apiUrl = `https://newsapi.org/v2/top-headlines?category=sports&language=en&pageSize=${PAGE_SIZE}&page=${currentPage}`;

          if (searchQuery) {
            apiUrl = `https://newsapi.org/v2/top-headlines?q=${searchQuery}&language=en&category=sports&apiKey=${API_KEY}&pageSize=${PAGE_SIZE}&page=${currentPage}`;
          }
