...
const BusinessPage = () => {
  ...
  useEffect(() => {
    const fetchBusinessNews = async () => {
      try {
        setLoading(true);
        let cachedData = localStorage.getItem(`businessNews-${searchQuery}`);
        ...
        if (
          ...
          let apiUrl = `https://newsapi.org/v2/top-headlines?category=business&language=en&pageSize=${PAGE_SIZE}&page=${currentPage}`;

          if (searchQuery) {
            apiUrl = `https://newsapi.org/v2/top-headlines?q=${searchQuery}&language=en&category=business&apiKey=${API_KEY}&pageSize=${PAGE_SIZE}&page=${currentPage}`;
          }
