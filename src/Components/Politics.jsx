...
const PoliticsPage = () => {
  ...
  useEffect(() => {
    const fetchPoliticsNews = async () => {
      try {
        setLoading(true);
        let cachedData = localStorage.getItem(`politicsNews-${searchQuery}`);
        ...
        if (
          ...
          let apiUrl = `https://newsapi.org/v2/top-headlines?category=politics&language=en&pageSize=${PAGE_SIZE}&page=${currentPage}`;

          if (searchQuery) {
            apiUrl = `https://newsapi.org/v2/top-headlines?q=${searchQuery}&language=en&category=politics&apiKey=${API_KEY}&pageSize=${PAGE_SIZE}&page=${currentPage}`;
          }
